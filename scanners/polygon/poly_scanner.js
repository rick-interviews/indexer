...
const analyzerService = `${process.env.SERVICES_HOST}:${process.env.ANALYZER_POLYGON_PORT}`;
...
const setupWeb3 = () => {
  return new Promise((resolve, reject) => {
    web3 = new Web3(new Web3.providers.HttpProvider(`${process.env.INFURA_POLY_REST}/${process.env.INFURA_API_KEY}`));
    resolve();
  });
}
...
const getLatestBlockDB = () => {
    return new Promise(async (resolve, reject) => {
      const sql = "SELECT max(block_height) as block_height FROM poly_blocks;";
...
const sendTransactionAnalyzer = fieldsToUpdate => {
  return new Promise((resolve, reject) => {
    request.post({
      ...
      url: `${analyzerService}/polygon/transactions/new`,
      body: fieldsToUpdate,
      json: true
  ...
  return new Promise((resolve, reject) => {
    web3.eth.getBlock(blockNumber, true, async (error, block) => {
      ...
      if (block && block.number != undefined) {
        ...
        await dbModule.updateOrCreateRow('poly_blocks', 'block_height', block.number, fieldsToUpdate);
        resolve();  
      }
    }).catch(error => {
      reject(error);
